...
class ZoomDataset(Dataset):
    ...
    def __init__(self, config, transforms = None):
        ...
        # Cache the specific transforms we need
        self.ground_transform = self.transforms.get("ground")
        self.sat_transform = self.transforms.get("satellite")
        self.sat_transform_no_aug = self.transforms.get("satellite_no_aug")
    ...
    def _load_ground_image(self, image_id):
        ...
        transform = self.ground_transform # Use cached transform
    ...
    def _load_satellite_sequence(self, sequence):
        ...
        for i, patch_index in enumerate(sequence):
            ...
            # Use the non-augmented transform for the first image (i == 0)
            # Use the (potentially) augmented transform for all subsequent images
            transform_to_apply = self.sat_transform_no_aug if i == 0 else self.sat_transform
            
            if transform_to_apply:
                patches.append(transform_to_apply(image))
            else:
                patches.append(to_tensor(image)) # Fallback
